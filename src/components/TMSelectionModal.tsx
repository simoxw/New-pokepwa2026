...
import { Search, X, Loader2, Sparkles, Check } from 'lucide-react';
...
export const TMSelectionModal: React.FC<TMSelectionModalProps> = ({
  ...
  useEffect(() => {
    ...
    async function loadMoves() {
      ...
      // Translate move names to Italian and extract details for type badges & accurate stats
  ...
  const filteredMoves = moves.filter(m => {
    ...
    const cat = (m.moveDetails?.category || '').toLowerCase();
    return (
  ...
  const handleChooseMove = async (moveInfo: { name: string; url: string }) => {
    try {
      ...
      console.error('Error teaching move:', e);
      const fallback = getMoveByName(moveInfo.name);
      onSelectMove(fallback);
    } finally {
  ...
  return (
    <div className="fixed inset-0 z-[120] bg-black/80 backdrop-blur-sm flex items-center justify-center p-3 sm:p-4">
      <div className="bg-white w-full max-w-lg rounded-3xl overflow-hidden shadow-2xl flex flex-col max-h-[90vh] border border-gray-100 animate-in fade-in zoom-in duration-200">
        ...
        <div className="p-4 sm:p-5 bg-gradient-to-r from-purple-700 via-indigo-700 to-purple-800 text-white flex items-center justify-between shadow-md">
          <div className="flex items-center gap-3">
            <div className="w-12 h-12 bg-white/10 rounded-2xl flex items-center justify-center p-1 backdrop-blur-md border border-white/20 shrink-0">
              <img 
            ...
            <div>
              <div className="flex items-center gap-2">
                <span className="text-[10px] font-black uppercase tracking-widest bg-purple-500/40 text-purple-200 px-2 py-0.5 rounded-full border border-purple-400/30">
                  Macchina Tecnica (MT)
                </span>
              </div>
              <h3 className="font-black text-lg leading-tight uppercase mt-0.5">
                Insegna Mossa a {pokemon.nickname || pokemon.name}
              </h3>
            </div>
          </div>
          <button 
        ...
        {/* Current Pokemon Moves Recap */}
        <div className="px-4 py-2.5 bg-purple-50/60 border-b border-purple-100 flex items-center justify-between text-xs">
          <span className="font-bold text-gray-600">Mosse attuali:</span>
          <div className="flex gap-1.5 flex-wrap">
            {pokemon.moves.map((m, idx) => (
              <span 
                key={`${m.name}-${idx}`} 
                className="bg-white border border-purple-200 px-2 py-0.5 rounded-lg font-bold text-gray-700 text-[10px] shadow-2xs"
              >
                {m.name}
              </span>
            ))}
        ...
        {/* Search Bar */}
        <div className="p-3.5 bg-gray-50 border-b flex items-center gap-2">
          <div className="relative flex-1">
            ...
            <input 
              ...
              className="w-full pl-9 pr-4 py-2 bg-white border border-gray-300 rounded-xl text-xs sm:text-sm font-bold focus:outline-none focus:ring-2 focus:ring-purple-500"
            />
          </div>
          <div className="text-xs font-black text-gray-500 bg-white px-3 py-2 rounded-xl border shrink-0">
        ...
        <div className="flex-1 overflow-y-auto p-3.5 space-y-2 min-h-0">
          {loading ? (
            <div className="py-12 flex flex-col items-center justify-center text-purple-600 gap-3">
              ...
              <p className="font-bold text-sm text-gray-600">Caricamento mosse compatibili...</p>
            </div>
            ...
            filteredMoves.map((m) => {
              ...
              const catBadge = getCategoryBadge(m.moveDetails?.category, m.moveDetails?.power);
                ...
                >
                  <div className="flex items-center gap-3 min-w-0 flex-1">
                    ...
                    <div className="min-w-0 flex-1">
                      {/* Move Name & Type Badge + Category Badge */}
                      <div className="flex items-center gap-1.5 flex-wrap mb-1">
                        <span className="font-black text-xs sm:text-sm text-gray-800 capitalize leading-tight">
                        ...
                        <span className={`text-[9px] font-black px-1.5 py-0.5 rounded-md border flex items-center gap-0.5 uppercase ${catBadge.className}`}>
                      ...
                      <div className="flex items-center gap-1.5 text-[10px] text-gray-500 font-bold flex-wrap">
                        {m.moveDetails?.power ? (
                          <span className="bg-gray-100 text-gray-700 px-1.5 py-0.5 rounded border border-gray-200/80 font-mono">
                            Pot: <strong>{m.moveDetails.power}</strong>
                          </span>
                        ) : (
                          <span className="bg-gray-100 text-slate-600 px-1.5 py-0.5 rounded border border-gray-200/80">
                            Stato (0 Danno)
                          </span>
                        )}

                        {m.moveDetails?.accuracy && (
                          <span className="bg-gray-100 text-gray-600 px-1.5 py-0.5 rounded border border-gray-200/80 font-mono">
                            Acc: {m.moveDetails.accuracy}%
                          </span>
                        )}
                  ...
                  {!alreadyLearned && (
                    <button 
                      ...
                      className="bg-purple-600 hover:bg-purple-700 text-white font-black text-xs px-3.5 py-2 rounded-xl shadow-md active:scale-95 transition-transform flex items-center gap-1 shrink-0 cursor-pointer"
                    >
                      {isSelected ? (
                        <>
        ...
        {/* Footer */}
        <div className="p-3.5 bg-gray-50 border-t flex justify-end">
          <button 
            onClick={onClose}
            className="px-5 py-2 bg-gray-200 hover:bg-gray-300 font-black text-xs text-gray-700 rounded-xl transition-colors cursor-pointer"
          >
            Annulla
          </button>
        </div>
